...
export default function AgentLogin({ searchParams }: { searchParams: { error?: string } }) {
  const s = getSession();
  if (s?.role === "agent") redirect("/quan-ly");
    ...
    <main className="grid min-h-screen place-items-center bg-brand-dark px-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-xl">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-3 font-heading text-2xl text-brand-dark">Cổng Đại lý</h1>
          <p className="text-xs text-ink/50">Quản lý cửa hàng của bạn</p>
        </div>

        {searchParams.error && (
          <div className="mb-4 rounded-lg border border-cta/40 bg-red-50 px-3 py-2 text-sm text-cta">
            {searchParams.error === "role" ? "Tài khoản này không phải đại lý." : "Email hoặc mật khẩu không đúng."}
          </div>
        )}
        ...
        <form action={login} className="space-y-3">
          ...
          <div>
            ...
            <input name="email" type="email" required defaultValue="minhanh@fohoway.vn" className="w-full rounded-lg border border-black/15 px-3 py-2.5 text-sm" />
          </div>
          <div>
        ...
        <p className="mt-4 text-center text-[11px] text-ink/40">Demo: minhanh@fohoway.vn / agent123</p>
      </div>
    </main>
  );
}
